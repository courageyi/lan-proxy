...
DSH Web GUI 默认只监听 `127.0.0.1:3080`，且 Web 载体没有内置认证层（`--host 0.0.0.0` 被有意禁用）。要安全地让局域网内其他设备使用 GUI，需要在 dsh 前面架一层独立的认证反向代理：未登录请求一律引导到登录页，认证通过后由代理转发给本机 dsh。
...
局域网设备 ──HTTPS──> [lan-proxy :3081] ──认证通过──> HTTP ──> dsh web :127.0.0.1:3080
                        ...
                        └─ 转发（普通 HTTP + WebSocket，改写 Host/Origin）
```
...
- **局域网多人共享**：同一可信局域网内，多台设备 / 多位成员需要访问本机 DSH 智能体界面，且要求先登录才能使用
- **访问门槛**：DSH 能执行任意命令，直接暴露到局域网等同开放远程执行权限；用密码做第一道门槛
- **不动 dsh 本体**：dsh 保持默认监听、源码与运行状态不变、无需重启，认证与暴露逻辑全部收敛在代理层
- **临时安全分享**：演示、评审等场景临时开放，分发 `credentials.txt` 中的账号，用后结束进程并删除防火墙规则即可收回
- **HTTPS 加密传输**：局域网内有中间人风险或组织要求加密时，用自签名 TLS 加密浏览器与代理之间的流量

> ⚠️ 不适合**公网暴露**：证书自签名、无自动续期，密码是唯一安全屏障，不支持多因素认证；也不提供每用户权限分级（所有账号权限相同）。
